...
\mypar{Rectangular layout}
  ...
  Any line segment that is not extended farther on either side is a \emph{maximal segment}.
...
\mypar{Overview}
  ...
  Once we have the preliminaries out of the way we show Theorem \ref{fix:th:family} in Section \ref{s:fix} and Theorem \ref{th:dsided} in Section \ref{s:algo}. The proof of Theorem \ref{fix:th:family} is by counterexample while the proof of Theorem \ref{th:dsided} is provided by giving a constructive algorithm.
