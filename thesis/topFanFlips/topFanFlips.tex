...
In Section \ref{ss:flipBlueZ} we obtained a vertically one-sided regular edge labeling (Lemma's \ref{lm:sweep:REL} and \ref{lm:sweep:vertOnsided}).
Moreover this regular edge labeling never has a split next to a topfan handle along the rightmost edge of the split (Lemma \ref{lm:sweep:NoTwoSplitsAboveEachOtherVertOnesided}). \fxwarning{TODO What is a topfan/fan}
