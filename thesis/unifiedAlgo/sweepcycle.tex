...
\section{Sweepcycle algorithm}
  ...
  \subsection{Irregularities}
    ...
    When the sweepcycle is no longer separating we can't update it anymore. However at this point it is easy to color the rest of the graph. All vertices in $\C$ must be adjacent to $\pS$ since $\pS \pW$ and $\pS \pE$ are part of $C$ by Invariant \ref{i:uni:SWandSE} $C$ has no chords by Invariant \ref{i:uni:noChords} and $C$ does not contain any interior vertices. Sice all vertices are adjacent to $\S$ all interior edges are adjacent to $\pS$.
