...
%  \begin{defi}[Prefence]
%    A prefence $\P$ is a interior path of $\C$ starting at $v_i \in \C$ and ending
%    at $v_j \in \C$ a both adjacent to $\pS$
%    \begin{enumerate}
%      \itemsep=-4pt
...
%      \item  The paths $v_1 v_2 \ldots v_i$ and $v_j v_{j+1} \ldots v_n$
%      have no chords nor 2chords on the right     \label{p:Cchordfree}
%    \end{enumerate}
%  \end{defi}
  ...
  \subsection{Irregularities}
    ...
    The \emph{range} of an irregularity will be given by its start and end index. Depending on what irregularities we find on the prefence we will update the sweepcycle with a different path. We will call this path the \emph{update path}. How this update works exactly will be shown in Section \ref{ss:sweep:Update}.

    \mypar{We have no irregularity}
      When there are no irregularities we update the sweepcycle with the prefence entire $\P$.

    \mypar{We have any chord} \mbox{ }
