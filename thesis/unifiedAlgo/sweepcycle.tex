...
\section{Sweepcycle algorithm}
  ...
  \subsection{Irregularities}
    ...
    \begin{lemma}
      ...
      Let $v$ be any splitvertex. Then the subsequent vertex on the rightmost path $w$ can't be the handle of a large topfan.
    \end{lemma}
    ...
    \begin{proof}
      ...
      Suppose that $w$ is the handle of a big topfan on the rightmost path

      Note that if $v$ is a splitvertex because it is adjacent to $\pS$ then since $w$ is on the rightmost path it also has to be adjacent to $\pS$. \fxwarning{TODO figure, something about interior vertex conditon}
      Hence $w$ is not the handle of a large topfan.


      On the other hand if the path $\P = vw\ldots x$ started due to a chord then this chord is given by $v a b x$.

      The interior of the rightmost path $\P$ and  $vabx$ has no vertices. Otherwise $\P$ would not be the rightmost path since any augmenting path would find $ab$ as chord.

      Now $w$ can't be connected to any vertex in $\P$ that would give a chord. So $w$ can only be connected to $a$ and $b$ and is thus a topfan of size at most $2$.
    \end{proof}
    ...
    We then flip all blue $Z$'s. To be more exact, the midlle edges.
    ...
    \begin{proof}
      The edges of $\P$ and $ab$ are not midlle edges of any $Z$. Hence they are not flipped. The same proof still holds.
    \end{proof}
