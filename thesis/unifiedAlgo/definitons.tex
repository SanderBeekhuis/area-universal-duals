...
\section{Some Definitions}
  ...
  \caption{}
  \label{fig:}
\end{figure}      \begin{subfigure}[b]{0.5\linewidth}
      ...
      In case $(b)$ we note that $p_i w$ and $p_i p_{i+1}$ are edges subsequent in clockwise order,\begin{figure}[h]
  \centering
  \includegraphics[scale=1]
  \caption{}
  \label{fig:}
\end{figure} hence $wp_{i+1}$ is also an edge. Hence $w$ is the first vertex adjacent to $p_{i+1}$ subsequent to $v_i$ in the clockwise rotation. Thus $w= w'$. They are duplicates and one of them must have been removed.
    ...
    \end{lemma}
    \begin{proof}
      Suppose that the right neighbor walk is crossing at a vertex $w= w_i =w_j$. Then one of $w_{j-1}$ and $w_{j+1}$ is in the clockwise interval $[w_{i-1}, w_{i+1} ]$ at the rotation at\begin{figure}[h]
  \centering
  \includegraphics[scale=1]
  \caption{}
  \label{fig:}
\end{figure} the triangle containing $w$ is a separating triangle.
  ...
  \caption{}
  \label{fig:}
\end{figure}  \fxnote{Actullly W rev(P)}
