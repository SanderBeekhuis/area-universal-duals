...
\mypar{Regular edge labellings}
  ...
  \mypar{$\mathbf{st}$-planar graphs}
    ...
    Hence a one-sided maximal segment corresponds to a face with one boundary path of length $2$ (A boundary path can not have length $1$ since by construction of a regular edge labeling it encloses a maximal segment and thus has to go trough at least one intermediate rectangle/vertex.) and a $k$-sided maximal segment to face with a shortest boundary path of length at most $k+1$.
